...
describe('Core: Unit Tests', () => {
    it('getPDFVersion', () => {
        ...
        expect(doc.getPdfVersion()).toEqual('1.3');
    });

    it('setPDFVersion', () => {
        const doc = new Document();
        expect(doc.getPdfVersion()).toEqual('1.3');
        doc.setPdfVersion('1.5');
        expect(doc.getPdfVersion()).toEqual('1.5');
    });

    /*
  it('private function getPageFormats', () => {
    const doc = new Document();
    expect(Object.keys(doc.getPageFormats()).length).toEqual(41);
  })

  it('private function getPageFormat', () => {
    const doc = new Document();
    expect(doc.getPageFormat('a4')[0]).toEqual(595.28);
    expect(doc.getPageFormat('a4')[1]).toEqual(841.89);
  })
